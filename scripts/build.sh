#!/bin/bash
# Builds the shkit distribution into dist/:
#   dist/shkit/                  staged dist tree (= tarball contents):
#     shkit.sh          bundle — src/lib modules concatenated in
#                                dependency order, shdoc blocks stripped
#     shkit.sh.map      line-range map for issue traceability
#     rnfshk.sh, install.sh, commands/, VERSION
#   dist/shkit.tar.gz   tarball of dist/shkit contents
#   dist/shkit.sh(.map) loose bundle assets for direct sourcing
...
{
  printf '#!/bin/sh\n'
  printf '# shkit.sh — generated bundle\n'
  printf '# Version: %s\n' "${VERSION}"
  printf '# Built:   %s\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
  printf '# Commit:  %s\n' "$GIT_SHA"
  printf '# Source:  https://github.com/%s/shkit\n' "$GITHUB_ORG"
  printf '\n'
...
tar -czf "${REPO_ROOT}/dist/shkit.tar.gz" -C "${STAGE}" .
sha256_of "${REPO_ROOT}/dist/shkit.tar.gz" >"${REPO_ROOT}/dist/shkit.tar.gz.sha256"
...
printf 'Staged:   %s\n' "$STAGE"
printf 'Tarball:  %s\n' "${REPO_ROOT}/dist/shkit.tar.gz"
printf 'Checksum: %s\n' "${REPO_ROOT}/dist/shkit.tar.gz.sha256"
printf 'Bundle:   %s\n' "${REPO_ROOT}/dist/shkit.sh"
